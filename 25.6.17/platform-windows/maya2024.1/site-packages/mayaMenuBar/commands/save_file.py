"""Command to save Maya scene with versioning."""
import os
import re
...
class SaveSceneWindow(QDialog):
    ...
    def on_save_clicked(self):
        ...
        version = target_filename.split('_')[-2]  # Extract version (e.g., "v001")
        ...
        try:
            ...
            QMessageBox.critical(self, "Folder Creation Failed", 
                                f"Could not create directories:\n{parent_dir}\nError: {str(e)}")
            return

        # First save (v001) - create project structure
        if version == "v001":
            project_dir = os.path.dirname(self.HAL_TASK_ROOT)
            maya_project_folders = ["_workarea", "_sculpts"]
            for folder in maya_project_folders:
        ...
        confirm = QMessageBox.question(
            self,
            "Confirm Save",
            f"Are you sure you want to save Maya file to:\n{absolute_path}",
