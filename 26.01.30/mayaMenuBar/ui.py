...
def create_menu():
 ...
 ##################################################################################   
    ...
    cmds.menuItem(
        label="Remove Shader Subsets",
        command=lambda *args: utils.executeDeferred(COMMANDS['remove_shader_subsets']),
        parent=shader_menu
    )
    cmds.menuItem(
        label="Auto Texture Repath",
        command=lambda *args: utils.executeDeferred(COMMANDS['auto_texture_repath']),
        parent=shader_menu
    )
...
###################################################################################
 ...
 ##################################################################################   
    ...
    # Reference to Import Submenu under UTILS
    cmds.menuItem(
        label="Reference to Import",
        command=lambda *args: utils.executeDeferred(COMMANDS['reference_to_import']),
        parent=utils_menu
    )
