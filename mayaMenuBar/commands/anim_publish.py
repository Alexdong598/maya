...
class PublishToolWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.resize(680, 610)
        self.setWindowTitle("Animation Publish Tool")

        # Load UI file
        script_dir = os.path.dirname(os.path.abspath(__file__))
        maya_menu_dir = os.path.dirname(script_dir)  # Go up to mayaMenuBar
        ui_file = os.path.join(maya_menu_dir, "QtWindows", "anim_publish_tool.ui")
        ...
        # Set up drag-drop for camera selection
        self.ui.camExportArea.setAcceptDrops(True)
        self.ui.camExportArea.dragEnterEvent = self._drag_enter_event
        self.ui.camExportArea.dropEvent = self._drop_event
    ...
    def set_sg_frame_range(self):
        from mayaMenuBar.utils.SGlogin import ShotgunDataManager
        ...
        """Set frame range to be specific from SG when SG Import button is clicked"""
        self.ui.startFrameEdit.setText(str(cut_in))
        self.ui.endFrameEdit_2.setText(str(cut_out))
    ...
    def publish(self):
        ...
        try:
            ...
            # Export playblast if camera export is enabled
            self._export_playblast(next_version)
    ...
    def _drag_enter_event(self, event):
        """Handle drag enter event for camera selection"""
        if event.mimeData().hasText() and event.buttons() == QtCore.Qt.MiddleButton:
            event.acceptProposedAction()
        else:
            event.ignore()

    def _drop_event(self, event):
        """Handle drop event for camera selection"""
        ...
        if mime_data.hasText():
            # Get the dragged object name from Maya
            dragged_obj = mime_data.text().strip()
            
            # Check if it's a camera shape node
            if cmds.objectType(dragged_obj) == 'camera':
                camera_shape = dragged_obj
            else:
                # Find all camera shape nodes under the transform
                camera_shapes = cmds.listRelatives(dragged_obj, 
                                                 allDescendents=True,
                                                 type='camera') or []
                
                if not camera_shapes:
                    QMessageBox.warning(self, "Invalid Selection",
                                      "No camera found under selected object")
                    return
                
                # Get the first camera shape (lowest in hierarchy)
                camera_shape = camera_shapes[0]
            
            # Get full path and format with pipes (but no leading pipe)
            full_path = cmds.ls(camera_shape, long=True)[0]
            display_name = full_path.lstrip('|')
            self.ui.camExportArea.setText(display_name)
            event.acceptProposedAction()

    def _export_playblast(self, version):
        """Export framestone playblast when camera export is enabled"""
        if not self.ui.exportCamOrNot.isChecked():
            return
            
        camera_path = self.ui.camExportArea.text().strip()
        if not camera_path:
            QMessageBox.warning(self, "Warning", "No camera selected for playblast")
            return
            
        # Get environment variables
        HAL_TASK_OUTPUT_ROOT = os.environ.get("HAL_TASK_OUTPUT_ROOT")
        if not HAL_TASK_OUTPUT_ROOT:
            QMessageBox.warning(self, "Error", "HAL_TASK_OUTPUT_ROOT environment variable not set")
            return
        ...
        # Create output path
        file_name = f"{HAL_PROJECT_ABBR}_{HAL_SEQUENCE}_{HAL_SHOT}_{HAL_TASK}_{version}_{HAL_USER_ABBR}.jpg"
        output_dir = f"{HAL_TASK_OUTPUT_ROOT}/playblast/{version}"
        output_path = f"{output_dir}/{file_name}".replace(os.sep, "/")
    ...
    def export_file(self, fmt, path, start_frame, end_frame):
        if fmt == "usdc":
            ...
            from mayaMenuBar.utils import exportABC
            importlib.reload(exportABC)
            exportABC.exportABC(path=path, start_frame=start_frame, end_frame=end_frame)
        elif fmt == "obj":
            QMessageBox.warning(self, "Warning", "OBJ format doesn't support animation export")
            return
