...
def check_uv_overlap(selection=None):
    """Check for overlapping UVs recursively on selected hierarchy"""
    overlapFaces = []
    oStr = ''
    ...
    for mesh in meshes:
        ...
        for uv in polyUVSet(mesh, q=1, auv=1):
            ...
            of = getOverlapUVFaces(str(mesh))
            if of:
                oStr += f"{mesh} has {len(of)} overlapped faces in uvset {uv}\n"
                overlapFaces.extend(of)
        polyUVSet(mesh, cuv=1, uvSet=str(curUV))

    if not overlapFaces:
        oStr = "No overlapping UV faces found!"
    else:
        oStr = "Found overlapping UV faces:\n" + oStr
        select(overlapFaces, r=1)

    confirmDialog(button="Ok", m=oStr, t="UV Overlap Check")
    return overlapFaces
