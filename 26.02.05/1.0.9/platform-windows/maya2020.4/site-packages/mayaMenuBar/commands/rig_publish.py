...
def load_ui(ui_file):
    ...
    if not file.open(QtCore.QFile.ReadOnly):
        if PY2:
            raise RuntimeError("Cannot open UI file: {} (check path)".format(ui_file))
        else:
...
class RigPublishToolWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        if not os.path.exists(ui_file):
            if PY2:
                raise RuntimeError("UI file not found at: {}".format(ui_file))
            else:
    ...
    def check_freeze_transform(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            if PY2:
                cmds.inViewMessage(msg="已冻结{}个节点的变换".format(len(unfrozen)), pos="topLeft", fade=True)
            else:
    ...
    def check_non_deformer_history(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            if PY2:
                cmds.inViewMessage(msg="已清理{}个节点的非变形历史".format(len(has_history)), pos="topLeft", fade=True)
            else:
    ...
    def name_space_checking(self):
        ...
        for obj in selected:
            ...
            if PY2:
                cmds.inViewMessage(msg="Cleaned {} namespaces".format(cleaned), pos="topLeft", fade=True)
            else:
    ...
    def auto_save_scene(self):
        ...
        try:
            ...
            if not current_file:
                ...
                if PY2:
                    cmds.warning("Scene was unsaved, auto-saved to: {}".format(temp_file_path))
                else:
            ...
            if PY2:
                cmds.error("Auto save failed: {}".format(str(e)))
            else:
    ...
    def publish(self):
        ...
        for obj in original_selection:
            if not cmds.objectType(obj) == 'transform':
                if PY2:
                    QMessageBox.warning(self, "Publish Warning", "Selected object '{}' is not a group (transform)".format(obj))
                else:
            ...
            if cmds.listRelatives(obj, parent=True):
                if PY2:
                    QMessageBox.warning(self, "Publish Warning", "Selected object '{}' is not top-level (has parent)".format(obj))
                else:
        ...
        try:
            ...
            if PY2:
                QMessageBox.information(
                    self,
                    "Publish Success",
                    "Published formats: {}\nVersion: {}".format(', '.join(selected_formats), next_version)
                )
            else:
                QMessageBox.information(
                    self,
            ...
            if PY2:
                QMessageBox.critical(self, "Publish Failed", "Error during publish: {}".format(str(e)))
            else:
    ...
    def create_and_submit_playblast(self, rig_path):
        ...
        try:
            ...
            if not camera or not cmds.objExists(camera):
                if PY2:
                    raise RuntimeError("Failed to create or find camera: {}".format(camera))
                else:
            ...
            if not os.path.exists(final_path):
                if PY2:
                    raise RuntimeError("Playblast file was not created at {}".format(final_path))
                else:
            ...
            if PY2:
                QMessageBox.warning(self, "Playblast/Shotgun Error", "Could not create/submit playblast:\n{}".format(e))
            else:
